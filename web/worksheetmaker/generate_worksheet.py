"""
Modified version of januschung's run.py to generate PDF into a byte string
"""
...
class MathWorksheetGenerator:
    ...
    def print_second_row_division(self, num_1: int, num_2: int):
        ...
        division_img_path = pathlib.Path(__file__).resolve().parent / 'img' / 'division.png'
        
        self.pdf.image(name=str(division_img_path), x=x_cor, y=y_cor)
...
def main(calc_type: str, digits: int, question_count: int):
    """Special function made for web version"""
    ...
    new_pdf = MathWorksheetGenerator(calc_type, size_, question_count)
    ...
    return new_pdf.pdf.output(dest='S').encode('latin-1')
